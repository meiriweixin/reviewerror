...
// Helper to get the best image URL for a question (cropped snippet preferred)
const getQuestionImageUrl = (question) => {
  // Prefer the cropped snippet image if available
  if (question.image_snippet_url) {
    return getImageUrl(question.image_snippet_url);
  }
  if (question.image_url) {
    return getImageUrl(question.image_url);
  }
  return null;
};
...
const Review = ({ user }) => {
  ...
  const [expandedPaperUrl, setExpandedPaperUrl] = useState(null); // Lightbox for uploaded papers
    ...
    <div className="max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {/* Uploaded papers - compact thumbnails */}
          {(() => {
            const seen = new Set();
            const uniqueImages = [];
            questions.forEach((q) => {
              ...
              if (url && !seen.has(url)) {
                seen.add(url);
                uniqueImages.push({ url, date: q.created_at, subject: q.subject });
              }
            });
            return uniqueImages.length > 0 ? (
              <div className="flex gap-3 overflow-x-auto pb-2">
                {uniqueImages.map((img, idx) => (
                  <div
                    key={`thumb-${idx}`}
                    onClick={() => setExpandedPaperUrl(getImageUrl(img.url))}
                    className="flex-shrink-0 w-28 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden cursor-pointer hover:shadow-md hover:border-blue-300 dark:hover:border-blue-700 transition-all group"
                  >
                    <div className="h-20 overflow-hidden dark:bg-white">
                      <img
                        src={getImageUrl(img.url)}
                        alt="Uploaded paper"
                        className="w-full h-full object-cover object-top"
                        style={{ filter: 'grayscale(100%) contrast(120%) brightness(105%)' }}
                      />
                    </div>
                    <div className="px-2 py-1.5 flex items-center gap-1">
                      ...
                      <span className="text-[10px] text-gray-500 dark:text-gray-400 truncate">Paper {idx + 1}</span>
                      <svg className="w-3 h-3 text-gray-300 dark:text-gray-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 flex-shrink-0 ml-auto transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {questions.map((question) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-1.5 text-gray-300 dark:text-gray-600 hover:text-red-500 dark:hover:text-red-400 transition-colors rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30"
              ...
              {(question.image_snippet_url || question.image_url) && (
      ...
      {selectedQuestion && (
        <div className="fixed inset-0 bg-black/60 dark:bg-black/80 flex items-center justify-center p-4 z-50" onClick={() => { setSelectedQuestion(null); setEditingNotes(false); setNotesText(''); setShowFeedback(false); setFeedbackText(''); }}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-slate-700 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              {(selectedQuestion.image_snippet_url || selectedQuestion.image_url) && (
      ...
      {expandedPaperUrl && (
        <div
          ...
          onClick={() => setExpandedPaperUrl(null)}
        >
          <div className="relative max-w-4xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            <div className="absolute -top-2 -right-2 z-10 flex gap-2">
              <button
                onClick={async () => {
                  try {
                    const res = await fetch(expandedPaperUrl);
                    const blob = await res.blob();
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `paper_${Date.now()}.jpg`;
                    ...
                    window.open(expandedPaperUrl, '_blank');
                  }
                }}
                className="p-2 bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 rounded-full shadow-lg hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-slate-700 dark:hover:text-blue-400 transition-colors"
              ...
              <button
                onClick={() => setExpandedPaperUrl(null)}
                className="p-2 bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 rounded-full shadow-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                title="Close"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            ...
            <div className="bg-white dark:bg-white rounded-2xl overflow-hidden shadow-2xl">
              <img
                src={expandedPaperUrl}
                alt="Full uploaded paper"
                className="w-full"
                style={{ filter: 'grayscale(100%) contrast(120%) brightness(105%)' }}
              />
            </div>
